	...
	<div id="wb_Form1" style="position:absolute;left:530px;top:350px;width:460px;height:300px;z-index:40;">
		<form  method="post"  action="#" class="form-horizontal" >
			<?php
				...
				{
					$consulta="SELECT id_zona,nombre_zona FROM zona_com WHERE id_zona='".$_POST["sel_ced"]."'";
					$res=consultar($consulta);
					$fila=mysql_fetch_array($res);
					$_POST["inputCedula"] =$fila[id_zona];
					$_POST["inputNombre"] =$fila[nombre_zona];
				}
						
				if(isset($_POST['b_modificar']))
				{
					$id_zona=$_POST["inputCedula"];
					$nombre_zona=$_POST["inputNombre"];
					$act="UPDATE zona_com SET id_zona='$id_zona',nombre_zona='$nombre_zona' WHERE id_zona='$id_zona'";
					if (!actualizar_bd($act)) 
					{
    					echo "<div class=\"alert alert-error\">";
  						echo "Error la Zona Comercial no se ha podido modificar!";
						echo "</div>";
					}
					else
					{
						echo "<div class=\"alert alert-success\">";
  						echo "La Zona ".$nombre_zona." ha sido Modificado exitosamente!";
						echo "</div>";
					}
				}
			...
			<div id="wb_Text3" style="position:absolute;left:60px;top:45px;width:430px;height:29px;z-index:0;text-align:left;">
				<span style="color:#000000;font-family:Arial;font-size:24px;">
					<strong>
						Datos de la Zona a Modificar
					</strong>
				</span>
			</div>
			...
			<div class="control-group" style="position:absolute;left:215px;top:80px;width:198px;height:18px;line-height:18px;z-index:1;">
				<select name="sel_ced" class="span2">
					<?php
						...
						$consulta="SELECT id_zona,nombre_zona FROM zona_com WHERE id_zona='$_POST[inputCedula].'";
						$res=consultar($consulta);
						while($row=mysql_fetch_array($res))
						{
							echo"<option selected value=\"$row[id_zona]\">$row[nombre_zona]</option>";
						} 
						$consulta="SELECT id_zona,nombre_zona FROM zona_com ORDER BY id_zona";
						$res=consultar($consulta);
	    				while($row=mysql_fetch_array($res))
	    				{
			...
			<div id="wb_Text4"  style="position:absolute;left:-10px;top:80px;width:230px;height:19px;z-index:2;text-align:center;">
				<span style="color:#000000;font-family:Arial;font-size:17px;">
					<strong>
						Seleccione Zona Comercial
					</strong>
				</span>
			</div>
			...
			<div id="wb_Text5" for="inputNombre" style="position:absolute;left:50px;top:150px;width:150px;height:19px;z-index:3;text-align:left;">
				<span style="color:#000000;font-family:Arial;font-size:17px;" >
					<strong>
						Nombre Zona
					</strong>
