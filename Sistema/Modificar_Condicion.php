                    ...
                    <a href="index_chuto.php">
                        <span class="ca-icon">F</span>
							<div class="ca-content">
                                <h2 class="ca-main">Atr&aacute;s</h2>
	...
	<div id="wb_Form1" style="position:absolute;left:530px;top:350px;width:460px;height:300px;z-index:40;">
		<form  method="post"  action="#" class="form-horizontal" >
			<?php
				...
				{
					$consulta="SELECT id_condicion,nombre_cond FROM condicion_chuto WHERE id_condicion='".$_POST["sel_ced"]."'";
					$res=consultar($consulta);
					$fila=mysql_fetch_array($res);
					$_POST["inputCedula"] =$fila[id_condicion];
					$_POST["inputNombre"] =$fila[nombre_cond];
				}
						
				if(isset($_POST['b_modificar']))
				{
					$id_condicion=$_POST["inputCedula"];
					$nombre_cond=$_POST["inputNombre"];
					$act="UPDATE condicion_chuto SET id_condicion='$id_condicion',nombre_cond='$nombre_cond' WHERE id_condicion='$id_condicion'";
					if (!actualizar_bd($act)) 
					{
    					echo "<div class=\"alert alert-error\">";
  						echo "Error La condicion no se ha podido modificar!";
						echo "</div>";
					}
					else
					{
						echo "<div class=\"alert alert-success\">";
  						echo "La condicion ".$nombre_cond." ha sido Modificada exitosamente!";
						echo "</div>";
					}
				}
			?>
			...
			<div id="wb_Text3" style="position:absolute;left:30px;top:45px;width:430px;height:29px;z-index:0;text-align:left;">
				<span style="color:#000000;font-family:Arial;font-size:24px;">
					<strong>
						Datos de la condición a Modificar
					</strong>
				</span>
			</div>
			...
			<div class="control-group" style="position:absolute;left:215px;top:80px;width:198px;height:18px;line-height:18px;z-index:1;">
				<select name="sel_ced" class="span2">
					<?php
						$consulta="SELECT id_condicion,nombre_cond FROM condicion_chuto ORDER BY id_condicion";
						$res=consultar($consulta);
						while($row=mysql_fetch_array($res))
						{
