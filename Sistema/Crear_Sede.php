		...
		<form  method="post"  action="#" class="form-horizontal" >
			...
			<div id="wb_Text3" style="position:absolute;left:40px;top:45px;width:500px;height:29px;z-index:0;text-align:left;">
				<span style="color:#000000;font-family:Arial;font-size:24px;">
					<strong>
						Ingrese Datos de la Nueva Sede
					</strong>
				</span>
			</div>

			<input type="text"  placeholder="PDG" name="inputNomdpto" style="position:absolute;left:215px;top:120px;width:198px;height:18px;line-height:18px;z-index:1;">
			...
			<?php
				if(isset($_POST["b_agregar"]))
				{
					...
					$insertar="INSERT INTO sede (nombre_sede) VALUES('$nomdpto')";
					if (!actualizar_bd($insertar)) 
					{
    					$query ="SELECT nombre_sede FROM sede WHERE nombre_sede='$nomdpto'";
    					$resultado=mysql_query($query) or die (mysql_error());
						if (mysql_num_rows($resultado)>0)
						{
							echo "<div class=\"alert alert-error\">";
  							echo "Error La Sede ya Existe!";
							echo "</div>";
						}
						else
						{
							echo "<div class=\"alert alert-error\">";
  							echo "Error La Sede no se ha podido crear!";
							echo "</div>";}
					}
					else
					{
						echo "<div class=\"alert alert-success\">";
  						echo "La sede ".$nombre_sede." ha sido creado exitosamente!";
						echo "</div>";
					}
				}
			?>		
