...
<head>
	...
	<div id="agenda">
		<?php
			...
			$q1="select * from mantenimiento_chutos where month(Fecha_Mantenimiento)='".$elmes."' and year(Fecha_Mantenimiento)='".$elanio."'";
			...
			if ($f1=mysql_fetch_array($r1))
			{
				...
				{
					$ides[$h]=$f1["id"];
					$eventos[$h]=$f1["Fecha_Mantenimiento"];
					$titulos[$h]=$f1["Placal_chuto"];
					$h+=1;
				}
			...
			echo "<h2>Mantenimientos Realizados en el mes de ".$meses[$mesactual]." de ".$elanio."</h2>";
			echo $mostrar;
			...
			for ($i=1;$i<=$diasdespues;$i++)
			{
				...
				if ($i>=$primeromes && $i<=$tope) 
				{
					...
					if ($noagregar==false){ echo "'>$j<a href='javascript:mostrar(\"evento$j\")' title='Crear un Evento el ".fecha($compuesta)."' class='vtip'></a><form id='evento$j' method='post' action='".$_SERVER["PHP_SELF"]."' style='display:none'><select name='titulo' >"; $consulta="SELECT placal_chuto FROM chuto";$res = mysql_query($consulta);while($row=mysql_fetch_array($res)){echo "<option value='$row[placal_chuto]'>$row[placal_chuto]</option>";} echo"</select><input type='Submit' name='Enviar' value='Guardar' class='enviar' /><input type='hidden' name='guardarevento' value='Si' /><input type='hidden' name='fecha' value='$compuesta' /></form>";
					}else{ echo "'>$j<a href='javascript:mostrar(\"evento$j\")' title='Agregar un Evento el ".fecha($compuesta)."' class='vtip'></a><form id='evento$j' method='post' action='".$_SERVER["PHP_SELF"]."' style='display:none'><select name='titulo' >"; $consulta="SELECT placal_chuto FROM chuto";$res = mysql_query($consulta);while($row=mysql_fetch_array($res)){echo "<option value='$row[placal_chuto]'>$row[placal_chuto]</option>";} echo"</select><input type='Submit' name='Enviar' value='Guardar' class='enviar' /><input type='hidden' name='addevent' value='Si' /><input type='hidden' name='fechas' value='$compuesta' /></form>";}
					$sqlevent="select * from mantenimiento_chutos where Fecha_Mantenimiento='".$compuesta."' order by id";
					mysql_select_db($dbname);
					$revent=mysql_query($sqlevent);
					while($rowevent=mysql_fetch_array($revent))
					{
						echo "<p>$rowevent[Placal_chuto]<a href='".$_SERVER["PHP_SELF"]."?borrarevento=".$rowevent["id"]."' onClick=\"return confirm('&iquest;Confirmas la eliminaci&oacute;n del Evento?')\" title='Eliminar este Evento del ".fecha($compuesta)."' class='vtip'></a></p>";
					}
