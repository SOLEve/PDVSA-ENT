		...
		<form  method="post"  action="#" class="form-horizontal" >
			<?php
				if(isset($_POST['b_consultar']))
				{
					...
					$_POST["inputCedula"] =$fila[id_tipo];
					$_POST["inputNombre"] =$fila[nombre_tipo];
				}
						
				if(isset($_POST['b_modificar']))
				{
					$id_tipo=$_POST["inputCedula"];
					$nom_tipo=$_POST["inputNombre"];
					$act="UPDATE tipo_cliente SET id_tipo='$id_tipo',nombre_tipo='$nom_tipo' WHERE id_tipo='$id_tipo'";
					...
					else
					{
						echo "<div class=\"alert alert-success\">";
  						echo "El Tipo de cliente ".$nombre_tipo." ha sido Modificado exitosamente!";
						echo "</div>";
					}
				}
				...
				<div class="control-group" style="position:absolute;left:215px;top:80px;width:198px;height:18px;line-height:18px;z-index:1;">
					<select name="sel_ced" class="span2">
						<?php
							...
							$consulta="SELECT id_tipo FROM tipo_cliente WHERE id_tipo='$_POST[inputCedula].'";
							$res=consultar($consulta);
							while($row=mysql_fetch_array($res))
							{
